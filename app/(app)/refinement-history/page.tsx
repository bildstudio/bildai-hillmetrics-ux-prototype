...
import { RefinementHistoryGrid } from "@/components/refinement-history/refinement-history-grid"
...
export default function RefinementHistoryPage() {
  ...
  const handleClear = (param: string) => {
    ...
    params.delete(param)
    router.replace(`/refinement-history${params.toString() ? `?${params.toString()}` : ""}`)
  }
  ...
  const handleViewWorkflow = useCallback(
    ...
    [openBlade, closeTopBlade, handleViewRefinement],
  )

  return (
    <div className="p-0">
      <RefinementHistoryGrid
        status={status}
