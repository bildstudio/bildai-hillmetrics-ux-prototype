...
export default function ProcessingRunDetails({ processingId }: { processingId: number }) {
  const [data, setData] = useState<ProcessingHistoryData | null>(null)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const res = await fetch(
        `/api/processing-history/by-id?processingId=${processingId}`,
      )
  ...
  const formatDuration = (secs: number | null) =>
    secs != null ? `${(secs / 60).toFixed(1)} min` : "-"
        ...
        <AccordionItem value="basic" className={itemClass}>
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                <p className="text-sm text-gray-500">Fetching ID</p>
                <p className="font-medium break-all">{data.fetchingID ?? "-"}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-sm text-gray-500">Number of content</p>
                <p className="font-medium break-all">{data.numberOfProcessingContent ?? "-"}</p>
              </div>
              {data.errorMessage && (
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="font-medium break-all">{formatDuration(data.processingTimeInSeconds)}</p>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
