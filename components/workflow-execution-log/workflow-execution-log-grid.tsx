...
import { StepsBadge } from "./steps-badge"
import ColumnsPanel from "@/components/shared/ColumnsPanel"
import TableColumnHeader from "@/components/table-column-header"

type SortDirection = "asc" | "desc" | null
type ColumnKey = keyof WorkflowExecutionLogData | "view"
...
export const WorkflowExecutionLogGrid: React.FC<{
  fluxId: string
  fluxIdFilter?: number | null
  onClearFluxId?: () => void
  onContentClick?: (fetchingID: number) => void
  onViewClick?: (fetchingID: number, fluxId?: number, fluxName?: string) => void
  onViewProcessingClick?: (
    processingID: number,
    fluxId?: number,
    fluxName?: string,
  ) => void
  onViewNormalizationClick?: (
    normalizationID: number,
    fluxId?: number,
    fluxName?: string,
  ) => void
  onViewRefinementClick?: (
    refinementID: number,
    fluxId?: number,
    fluxName?: string,
  ) => void
  onViewCalculationClick?: (
    calculationID: number,
    fluxId?: number,
    fluxName?: string,
  ) => void
  onWorkflowDetailsClick?: (item: WorkflowExecutionLogData) => void
  ...
  /**
   * When true the columns showing the Flux ID and Flux Name will be hidden even
   * if `fluxId` is set to "all".
   */
  hideFluxColumns?: boolean
}> = ({
  fluxId,
  fluxIdFilter,
  onClearFluxId,
  ...
  hideFluxColumns = false,
}) => {
  const [data, setData] = useState<WorkflowExecutionLogData[]>([])
  const [totalCount, setTotalCount] = useState(0)
  const [isLoading, setIsLoading] = useState(true)

  const [sortColumn, setSortColumn] = useState<ColumnKey | null>(defaultSortColumn)
  ...
  const handleRenameConfirm = async () => {
    ...
    await fetch(`/api/reports/update?id=${id}`, {
      method: "PATCH",
  ...
  const handleMenuAction = (
    action: WorkflowMenuAction,
    row: WorkflowExecutionLogData,
  ) => {
    const fluxName =
      row.flux_name || fluxNames[row.flux_id] || `Flux ${row.flux_id}`
    switch (action) {
      case "viewWorkflow":
        onWorkflowDetailsClick?.(row)
        break
      case "quickFlux":
        setQuickFluxId(row.flux_id)
        break
      case "viewFlux":
        openViewBlade(String(row.flux_id), fluxName)
        break
      case "editFlux":
        handleEditFlux(row.flux_id)
        break
      case "renameFlux":
        handleRenameClick(row.flux_id)
        break
      case "viewFetching":
        row.fetching_id &&
          onViewClick?.(row.fetching_id, row.flux_id, fluxName)
        break
      case "viewProcessing":
        row.processing_id !== null &&
          onViewProcessingClick?.(row.processing_id, row.flux_id, fluxName)
        break
      case "viewNormalization":
        row.normalization_id &&
          onViewNormalizationClick?.(row.normalization_id, row.flux_id, fluxName)
        break
      case "viewRefinement":
        row.refinement_id &&
          onViewRefinementClick?.(row.refinement_id, row.flux_id, fluxName)
        break
      case "viewCalculation":
        row.calculation_id &&
          onViewCalculationClick?.(row.calculation_id, row.flux_id, fluxName)
        break
      case "viewContent":
        onContentClick?.(row.fetching_id)
        break
      default:
        break
    }
  ...
  const showFluxId = fluxId === "all" && !hideFluxColumns
  const [columns, setColumns] = useState<Column[]>([
    { key: "id", label: "ID", hidden: false },
    ...(showFluxId
      ? [
          { key: "flux_id", label: "Flux ID", hidden: false },
          { key: "flux_name", label: "Flux Name", hidden: false },
        ]
      : []),
    { key: "run_number", label: "Run Number", hidden: false },
    { key: "status", label: "Status", hidden: false },
    { key: "progress", label: "Progress", hidden: false },
    { key: "steps", label: "Completed stages", hidden: false },
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("workflow-execution-log-columns")
    if (stored) {
      try {
        let parsed: Column[] = JSON.parse(stored)
        if (!showFluxId) {
          parsed = parsed.filter(
            (c) => c.key !== "flux_id" && c.key !== "flux_name",
          )
        }
        setColumns(parsed)
      } catch {}
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && showFluxId) {
      localStorage.setItem(
        "workflow-execution-log-columns",
  ...
  const availableFilterFields: FilterField[] = [
    ...
    ...(showFluxId
      ? [
          { key: "flux_id", label: "Flux ID", type: "text" },
          { key: "flux_name", label: "Flux Name", type: "text" },
        ]
      : []),
    { key: "run_number", label: "Run Number", type: "text" },
    ...
    { key: "fetching_id", label: "Fetching ID", type: "text" },
    { key: "processing_id", label: "Processing ID", type: "text" },
    { key: "normalization_id", label: "Normalization ID", type: "text" },
    { key: "refinement_id", label: "Refinement ID", type: "text" },
    { key: "calculation_id", label: "Calculation ID", type: "text" },
  ]
  ...
  useEffect(() => {
    if (statusFilter) {
      ...
      setAppliedFilters((prev) => prev.filter((f) => f.id !== "status_summary"))
    }
  }, [statusFilter])

  // Handle duration bucket filter from Summary tab
  useEffect(() => {
    if (durationBucketFilter) {
      ...
      setAppliedFilters([...otherFilters, durationFilterObj])
      setCurrentPage(1)
    } else {
      setAppliedFilters((prev) => prev.filter((f) => f.id !== "duration_summary"))
    }
  }, [durationBucketFilter])
  ...
  const fetchData = useCallback(async () => {
    ...
    if (fluxIdFilter !== null && fluxIdFilter !== undefined) {
      serverFilters = serverFilters.filter((f) => f.field !== "flux_id")
      serverFilters.push({ field: "flux_id", operator: "equals", value: fluxIdFilter })
    }

    const params = new URLSearchParams()
    params.set("fluxId", fluxId)
    params.set("page", String(currentPage))
    params.set("pageSize", String(pageSize))
    if (sortColumn) params.set("sortColumn", sortColumn)
    if (sortDirection) params.set("sortDirection", sortDirection)
    if (serverFilters.length > 0) params.set("filters", JSON.stringify(serverFilters))

    const res = await fetch(`/api/workflow-execution-log?${params.toString()}`, {
      cache: "no-store",
    })
    ...
    if (error) {
      ...
      setData(data)
      if (showFluxId) {
        const fluxIds = Array.from(new Set(data.map((d) => d.flux_id)))
        const resNames = await fetch(
          `/api/reports/flux-names?ids=${fluxIds.join(',')}`,
          { cache: 'no-store' },
        )
        const { data: names } = await resNames.json()
        setFluxNames(names)
      } else {
        setFluxNames({})
      }
    ...
    setIsLoading(false)
  }, [fluxId, fluxIdFilter, currentPage, pageSize, sortColumn, sortDirection, appliedFilters])
  ...
  const renderCell = (item: WorkflowExecutionLogData, columnKey: ColumnKey) => {
    const value = item[columnKey]
    const fluxName = item.flux_name || fluxNames[item.flux_id] || `Flux ${item.flux_id}`
    ...
    switch (columnKey) {
      case "id":
        return (
          <Badge
            ...
            onClick={() => onWorkflowDetailsClick?.(item)}
          >
            {item.id}
          </Badge>
        )
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge
                  className="bg-black text-white hover:bg-gray-300 cursor-pointer rounded-full px-2"
                  onClick={() => openViewBlade(String(item.flux_id), fluxName)}
                >
                  {item.flux_id}
                </Badge>
              </TooltipTrigger>
      ...
      case "content_count":
        ...
        return (
          <Badge
            className="bg-black text-white hover:bg-gray-300 cursor-pointer rounded-full px-2"
            onClick={() => onContentClick?.(item.fetching_id)}
          >
            {contentCount}
          </Badge>
        )
      ...
      case "view": {
        return (
          <div className="flex items-center justify-end space-x-1 pr-2">
            {isLargeScreen &&
              (hoveredRowId === item.fetching_id || openDropdownRowId === item.fetching_id) && (
                <>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            size="icon"
                            className="h-8 w-8 text-gray-500 hover:bg-gray-100"
                            onClick={() => setQuickFluxId(item.flux_id)}
                          >
            ...
            <DropdownMenu onOpenChange={(o) => setOpenDropdownRowId(o ? item.fetching_id : null)}>
              ...
              <DropdownMenuContent className="w-64" align="end">
                <WorkflowMenuItems
                  ...
                  includeView
                  includeQuickFlux={includeQuickFlux}
                  includeFluxManagement={includeFluxManagement}
                  components={{
  ...
  const handleApplySpecificFilter = () => {
    ...
    switch (selectedFilterField.type) {
      ...
      case "duration":
        ...
        break
      case "timerange":
        const isStartTime = selectedFilterField.key === "started_at"
  ...
  function handleEditFilter(filter: AppliedFilter) {
    ...
    switch (field.type) {
      ...
      case "duration":
        ...
        break
      case "timerange":
        const isStartTime = field.key === "started_at"
  ...
  if (!isLoading && data.length === 0 && appliedFilters.length === 0) {
      ...
      return (
        <div className="flex items-center justify-center h-96">
          <div className="bg-muted rounded-lg h-64 w-full flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-gray-500">No workflow executions yet.</span>
          </div>
        </div>
      )
    }
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center text-gray-500">
        ...
        <p className="text-lg">No workflow execution history found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative">
        ...
        <CardContent className="p-0">
          <div className="overflow-auto">
            <DragDropContext onDragEnd={handleDragEnd}>
              <Table>
                <TableHeader>
                  <Droppable droppableId="columns-workflow-execution" direction="horizontal">
                    {(provided) => (
              ...
              <TableBody>
                {data.length > 0 ? (
                  data.map((item) => (
                    <ContextMenu
                      key={item.fetching_id}
                      open={openContextMenuRowId === item.fetching_id}
                      onOpenChange={(o) =>
                        setOpenContextMenuRowId(o ? item.fetching_id : null)
                      }
                    >
                      <ContextMenuTrigger asChild>
                        <TableRow
                          onMouseEnter={() => setHoveredRowId(item.fetching_id)}
      ...
      {includeQuickFlux && quickFluxId !== null &&
        createPortal(
