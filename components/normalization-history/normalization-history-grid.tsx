...
// Normalization data interface
interface NormalizationHistoryData {
  id: number
  flux_id: number
...
type ColumnKey = keyof NormalizationHistoryData | "view"
...
interface NormalizationHistoryGridProps {
  status?: string | null
  durationBucket?: string | null
  date?: string | null
  fluxId?: string | null
  onViewNormalization?: (id: number, fluxId?: number, name?: string) => void
  onViewWorkflow?: (item: NormalizationHistoryData) => void
}
...
const durationBuckets = [
  { value: "0-1min", label: "0-1 minute" },
  { value: "1-5min", label: "1-5 minutes" },
  { value: "5-15min", label: "5-15 minutes" },
  { value: "15-60min", label: "15-60 minutes" },
  { value: "60min+", label: "60+ minutes" },
]

export function NormalizationHistoryGrid({
  status,
  durationBucket,
  date,
  fluxId,
  onViewNormalization,
  onViewWorkflow,
}: NormalizationHistoryGridProps) {
  const [data, setData] = useState<NormalizationHistoryData[]>([])
  ...
  // Generate mock data
  const generateMockData = useCallback((): NormalizationHistoryData[] => {
    const mockData: NormalizationHistoryData[] = []
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      mockData.push({
        ...
        flux_name: `Normalization Flux ${100 + Math.floor(Math.random() * 50)}`,
        ...
        error_message: randomStatus === "Failed" ? `Normalization error ${Math.floor(Math.random() * 100)}` : null,
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError("Failed to load normalization history")
  ...
  const applyFilters = useCallback((data: NormalizationHistoryData[], filters: AppliedFilter[]) => {
    return data.filter(item => {
      return filters.every(filter => {
        const value = item[filter.field as keyof NormalizationHistoryData]
  ...
  const renderCell = (item: NormalizationHistoryData, columnKey: ColumnKey) => {
    const value = item[columnKey as keyof NormalizationHistoryData]
          ...
          <div className="flex items-center justify-end space-x-1 pr-2">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8 text-gray-500 hover:bg-gray-100"
                        onClick={() => onViewNormalization?.(item.id, item.flux_id, item.flux_name || undefined)}
                      >
            ...
            <DropdownMenu onOpenChange={(o) => setOpenDropdownRowId(o ? item.id : null)}>
              ...
              <DropdownMenuContent className="w-64" align="end">
                <DropdownMenuItem onClick={() => onViewNormalization?.(item.id, item.flux_id, item.flux_name || undefined)}>
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="flex items-center gap-2">
          ...
          <span>Loading normalization history...</span>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center text-gray-500">
        ...
        <p className="text-lg">No normalization history found.</p>
      </div>
    )
  }
          ...
          <CardContent className="p-0">
            <div className="overflow-auto">
              <DragDropContext onDragEnd={handleDragEnd}>
                <Table>
                  <TableHeader>
                    <Droppable droppableId="columns-normalization" direction="horizontal">
                      {(provided) => (
                    ...
                    {paginatedData.length > 0 ? (
                      paginatedData.map((item) => (
                        ...
                        >
                          ...
                          <ContextMenuContent>
                            <ContextMenuLabel>Normalization Actions</ContextMenuLabel>
                            <ContextMenuSeparator />
                            <ContextMenuItem onClick={() => onViewNormalization?.(item.id, item.flux_id, item.flux_name || undefined)}>
                              <Eye className="h-4 w-4 mr-2" />
                              View Details
                            </ContextMenuItem>
