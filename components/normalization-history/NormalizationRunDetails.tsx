...
import { getNormalizationHistoryById, type NormalizationHistoryData } from "@/app/actions/normalization-history"

export default function NormalizationRunDetails({ normalizationId }: { normalizationId: number }) {
  const [data, setData] = useState<NormalizationHistoryData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const res = await getNormalizationHistoryById(normalizationId)
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="font-medium break-all">{formatDuration(data.normalizationTimeInSeconds)}</p>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
