...
// Refinement data interface
interface RefinementHistoryData {
  id: number
  flux_id: number
...
type ColumnKey = keyof RefinementHistoryData | "view"
...
interface RefinementHistoryGridProps {
  status?: string | null
  durationBucket?: string | null
  date?: string | null
  fluxId?: string | null
  onViewRefinement?: (id: number, fluxId?: number, name?: string) => void
  onViewWorkflow?: (item: RefinementHistoryData) => void
}
...
export function RefinementHistoryGrid({
  status,
  durationBucket,
  date,
  fluxId,
  onViewRefinement,
  onViewWorkflow,
}: RefinementHistoryGridProps) {
  const [data, setData] = useState<RefinementHistoryData[]>([])
  ...
  // Generate mock data
  const generateMockData = useCallback((): RefinementHistoryData[] => {
    const mockData: RefinementHistoryData[] = []
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      mockData.push({
        ...
        flux_name: `Refinement Flux ${100 + Math.floor(Math.random() * 50)}`,
        ...
        error_message: randomStatus === "Failed" ? `Refinement error ${Math.floor(Math.random() * 100)}` : null,
  ...
  const applyFilters = useCallback((data: RefinementHistoryData[], filters: AppliedFilter[]) => {
    return data.filter(item => {
      return filters.every(filter => {
        const value = item[filter.field as keyof RefinementHistoryData]
  ...
  const renderCell = (item: RefinementHistoryData, columnKey: ColumnKey) => {
    const value = item[columnKey as keyof RefinementHistoryData]
          ...
          <div className="flex items-center justify-end space-x-1 pr-2">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8 text-gray-500 hover:bg-gray-100"
                        onClick={() => onViewRefinement?.(item.id, item.flux_id, item.flux_name || undefined)}
                      >
            ...
            <DropdownMenu onOpenChange={(o) => setOpenDropdownRowId(o ? item.id : null)}>
              ...
              <DropdownMenuContent className="w-64" align="end">
                <DropdownMenuItem onClick={() => onViewRefinement?.(item.id, item.flux_id, item.flux_name || undefined)}>
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="flex items-center gap-2">
          ...
          <span>Loading refinement history...</span>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center text-gray-500">
        ...
        <p className="text-lg">No refinement history found.</p>
      </div>
    )
  }
          ...
          <CardContent className="p-0">
            <div className="overflow-auto">
              <DragDropContext onDragEnd={handleDragEnd}>
                <Table>
                  <TableHeader>
                    <Droppable droppableId="columns-refinement" direction="horizontal">
                      {(provided) => (
                    ...
                    {paginatedData.length > 0 ? (
                      paginatedData.map((item) => (
                        ...
                        >
                          ...
                          <ContextMenuContent>
                            <ContextMenuLabel>Refinement Actions</ContextMenuLabel>
                            <ContextMenuSeparator />
                            <ContextMenuItem onClick={() => onViewRefinement?.(item.id, item.flux_id, item.flux_name || undefined)}>
                              <Eye className="h-4 w-4 mr-2" />
                              View Details
                            </ContextMenuItem>
