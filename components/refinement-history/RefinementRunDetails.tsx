...
import { getRefinementHistoryById, type RefinementHistoryData } from "@/app/actions/refinement-history"

export default function RefinementRunDetails({ refinementId }: { refinementId: number }) {
  const [data, setData] = useState<RefinementHistoryData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const res = await getRefinementHistoryById(refinementId)
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="font-medium break-all">{formatDuration(data.refinementTimeInSeconds)}</p>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
