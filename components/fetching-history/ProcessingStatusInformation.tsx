...
import { getProcessingStatusCountsByFetching, type ContentStatusCount } from "@/app/actions/fetching-content-history"

export default function ProcessingStatusInformation({
  fetchingId,
  onNavigate,
}: {
  fetchingId: number
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true)
        const res = await getProcessingStatusCountsByFetching(fetchingId)
        if (!res.error && res.data) {
          // Create completely safe copy to avoid immutable property issues
        ...
        // Silently handle 'enabled property' errors that don't affect functionality
        if (!error.toString().includes('enabled') || !error.toString().includes('getter')) {
          console.error("Error in ProcessingStatusInformation load:", error)
        }
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="w-full lg:w-1/2 bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow relative min-h-[420px]">
      <div className="space-y-2 mb-4">
        <h3 className="text-lg font-bold pr-2">Processing information</h3>
        <p className="text-sm text-gray-500">
          Overview of processings triggered by this fetching.{' '}
          <button 
