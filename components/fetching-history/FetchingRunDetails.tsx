...
import { getFetchingHistoryById, type FetchingHistoryData } from "@/app/actions/fetching-history"

export default function FetchingRunDetails({ fetchingId }: { fetchingId: number }) {
  const [data, setData] = useState<FetchingHistoryData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const res = await getFetchingHistoryById(fetchingId)
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="font-medium break-all">{data.numberOfContent ?? "-"}</p>
              </div>
              {data.errorMessage && (
          ...
          <AccordionContent className={contentClass}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="font-medium break-all">{formatDuration(data.fetchingTimeInSeconds)}</p>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
