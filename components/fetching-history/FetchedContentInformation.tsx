...
import { getFetchedContentStatusCounts, type ContentStatusCount } from "@/app/actions/fetching-content-history"

export default function FetchedContentInformation({
  fetchingId,
  onNavigate,
  overrideData,
}: {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true)
        const res = await getFetchedContentStatusCounts(fetchingId)
        ...
        if (!error.toString().includes('enabled') || !error.toString().includes('getter')) {
          console.error("Error in FetchedContentInformation load:", error)
        }
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="w-full lg:w-1/2 bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow relative min-h-[420px]">
      <div className="space-y-2 mb-4">
        <h3 className="text-lg font-bold pr-2">Fetched content information</h3>
        <p className="text-sm text-gray-500">
          Overview of statuses for fetched contents.{' '}
          <button 
